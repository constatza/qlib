...
class VQLS:
    ...
    def __init__(self, A, b, ansatz, backend=backend, num_shots=1):
        self.matrix = A
        self.target = b
        self.lcu = LinearDecompositionOfUnitaries(A)
        self.Ub = unitary_from_column_vector(b)
        self.ansatz = ansatz
        self.projector = LocalProjector(self.lcu, ansatz, self.Ub)
    ...
    def local_cost(self, values):
        ...
        self.cost = 1 - delta_sum/beta_norm/num_working_qubits/2
        return self.cost
...
class FixedAnsatz:
    ...
    def __init__(self, num_qubits, num_layers=1):
        ...
        self.num_parameters = 2*num_layers*(num_qubits//2 + (num_qubits-1)//2) + num_qubits
